...
pub(crate) fn classify_dns_query(packet: &[u8]) -> Option<DnsQuery<'_>> {
    ...
    // Reject fragments, inconsistent lengths and response packets at ingress.
    if packet[6] & 0x3f != 0
        || packet[7] != 0
        || total_len != packet.len()
        || ihl + udp_len != total_len
    {
        return None;
    }
...
mod tests {
    ...
    #[test]
    fn rejects_fragmented_truncated_and_response_packets() {
        let original = query("node.fips");
        for length in 0..original.len() {
            assert!(classify_dns_query(&original[..length]).is_none());
        }
        for index in [6, 7] {
            let mut p = original.clone();
            p[index] |= 1;
            assert!(classify_dns_query(&p).is_none());
        }
        let mut p = original.clone();
        p[30] |= 0x80;
        assert!(classify_dns_query(&p).is_none());
